"""
Enhanced Anomaly Video Evaluation for Qwen2.5-VL
================================================

This script evaluates Qwen2.5-VL's ability to detect and describe anomalies in videos.
It now uses ChatGPT (OpenAI) as the **only** judge—no manual/keyword scoring remains.

Requirements:
- pip install openai python-dotenv transformers pillow torch pandas numpy matplotlib seaborn
- .env file with OPENAI_API_KEY=... (OPTIONAL: OPENAI_JUDGE_MODEL, default 'gpt-4o-mini')
...
class AnomalyVideoEvaluator:
    def __init__(self, model_name: str = "Qwen/Qwen2.5-VL-7B-Instruct", verbose: bool = False):
    ...
    def run_evaluation(self, csv_path: str, video_dir: str, max_videos: int = None) -> Dict:
        print(f"🚀 Starting comprehensive anomaly video evaluation")
    ...
    def generate_plots(self, results: Dict, output_path: Path, timestamp: str):
        ...
        fig.suptitle('Qwen2.5-VL Anomaly Video Evaluation Results', fontsize=16, fontweight='bold')
...
def test_single_video():
    ...
    print("🧪 Testing single video evaluation...")
...
def main():
    ...
    print("🎬 Enhanced Anomaly Video Evaluation for Qwen2.5-VL (ChatGPT Judge)")
    print("=" * 60)
    ...
    print(f"📹 Evaluating up to {args.max_videos} videos")

    evaluator = AnomalyVideoEvaluator(verbose=args.verbose)
